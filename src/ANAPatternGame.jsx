...
import PATTERNS, { patternReferences } from './patterns.js';
...
function ANAPatternGame() {
  ...
  // Load the latest PubMed scan (generated by pubmed_scan.py). Optional file:
  // if it isn't there yet, Learn Mode just shows the curated references.
  useEffect(() => {
    fetch(`${import.meta.env.BASE_URL}research.json`)
      .then(r => (r.ok ? r.json() : {}))
      .then(setResearch)
      .catch(() => setResearch({}));
  }, []);
      ...
      <div className="min-h-screen bg-gray-900 text-white p-4 md:p-8">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-gray-800 rounded-lg p-5 md:p-8 mb-8">
            ...
            <div className="learn-card-refs">
              <p className="text-sm font-bold text-gray-200 mb-2">References</p>
              <ul className="text-sm text-blue-300 space-y-1">
                {patternReferences(pattern.id).map((ref) => (
                  <li key={ref.url}>
              ...
              {(research[pattern.id] || []).length > 0 && (
                <div className="mt-4">
                  <p className="text-sm font-bold text-gray-200 mb-2">
                    Recent research <span className="text-gray-400 font-normal">({research[pattern.id].length})</span>
                  </p>
                  <ul className="text-sm text-blue-300 space-y-1">
                    {research[pattern.id].map((paper) => (
                      <li key={paper.pmid}>
